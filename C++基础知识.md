...
这就相当于产生了一个二维数组`a[m][n]` 静态声明的数组可以有公式(假设也是M行N列)：`b[i][j] = b[i*n+j]` 

- 这是因为数组b是连续的一片内存，而动态声明的数组任意的`a[k]` 都是一个int *类型,即一个地址,所以只能`a[i][j]`或者是` *( *(a+i)+j)`来访问数组,而不能使用`a[i *n+j]`这样使用.
