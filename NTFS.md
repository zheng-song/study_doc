...
​	VCN对同属一个文件的所有簇进行编号，从0到m。NTFS依赖于虚拟簇号（VCN）来引用一个文件内部的数据。 VCN是带符号的整数，且不必是物理上连续的。它们可以被映射到该卷上的任何LCN编号，由Data Runs完成这个映射。
...
​	MFT中的基本文件记录有一个指针指向大小为2KB的运行(4KB是对应着4KB的簇尺寸)，那是一个非常驻索引缓冲区，包含着下一层的目录或文件。
