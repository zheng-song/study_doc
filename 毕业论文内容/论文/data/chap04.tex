...
\begin{figure}[!h]
\centering
\includegraphics[width=1.0\textwidth]{./graphics/RoutonLog.pdf}
\caption{主机端日志分析工具界面}\label{fig:RoutonLog}
\end{figure}\\
\textbf{串口读取流程}

当用户打开串口后，主窗口中的串口读取函数会对串口中的数据进行非堵塞地读取，并按协议格式进行解析、显示和存盘。
首先，解析一下协议格式，每种日志级别的信息在日志显示框中会以不同的底色显示。为了提供系统的可用性，对不按协议格式发送的日志信息，本系统也做了合理的容错处理,主要是针对没有调用Log接口而是直接使用标准输出重定向输出的调试信息。对串口读取的日志信息进行一下的分类处理：
